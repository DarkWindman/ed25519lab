...
def main() -> None:
    ...
    # --- codec, broken down ---
    bench("codec", "to_bytes_compressed", lambda: pt.to_bytes_compressed())
    bench("codec", "  _recover_x only", lambda: _recover_x(y_only, 0))
    bench("codec", "  subgroup check [L]P only", lambda: _mul_int(pt, GE.ORDER).infinity)
    bench("codec", "from_bytes_compressed (full)", lambda: GE.from_bytes_compressed(enc))
    ...
    # --- session sizing ---
    decode = next(ms for g, n, ms in ROWS if n == "from_bytes_compressed (full)")
